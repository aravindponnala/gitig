...
EnxRtc.getDevices(function (arg) {
        if (arg.result === 0) {
            ...
            for (i = 0; i < camLst.length; i++) {
                caml.push(camLst[i].deviceId);
            }
            for (j = 0; j < micLst.length; j++) {
                micl.push(micLst[i].deviceId);
            }
        }
}
);
var customerid = urlData.customerid
var doctorid = urlData.doctorid

console.log(doctorid)
console.log(customerid)


console.log(micl,'miclist')
console.log(caml,'camlist')
var SUPPORT_URL = "https://aodhpet.com/videothankyou/"+customerid +"/"+doctorid;
        ...
        localStream = EnxRtc.joinRoom(token, config, function (success, error) {
            ...
            if (success && success != null) {
                ...
                room.addEventListener('room-disconnected', function (event) {
                    ...
                    if (event.message == 'conference-expired') {
                        window.location.href = SUPPORT_URL;


                    } else if (event.message === 'unexpected-disconnection') {
                        networkMsgs.style.display = 'flex';
                        networkMsgs.innerHTML = '<div>Signing out...</div>';
                        window.location.href = SUPPORT_URL;
...
function videoidfun() {
    var vids = document.getElementsByClassName("vcx_stream classic_vcx_stream")
    var i = 0;
...
var change = document.getElementById("switchbttn");
var front_cam = true;
function switchbttn() {
    console.log(change.value, 'hdkjah')
    console.log(caml, 'camllist')
    if (front_cam == true) {
        console.log(caml[1], 'caml[1]')
        localStream.switchCamera(localStream,caml[1],function(stream){
            if (stream && stream.getID) {
                localStream = stream; // LocalStream updated  
            }
            else{
                console.log('somethng went worng')
                alert("This device does not support video switching we are working on it");
            }
        });

        // EnxRtc.switchMediaDevice(localStream, micl[0], caml[1], function (stream) {
        //     if (stream && stream.getID) {
        //         localStream = stream; // LocalStream updated  
        //     }
        //     else {
        ...
        console.log(caml[0], 'caml[0]')
        localStream.switchCamera(localStream, caml[0], function (stream) {
            if (stream && stream.getID) {
                localStream = stream; // LocalStream updated  
            }
            else {
